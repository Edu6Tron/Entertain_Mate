...
import { getCategoryLabels, getMonthKeys, matchesWatchlistFilters } from "@/lib/watchlistFilters";
...
function CollectionDashboard() {
  ...
  const revokeExtensionToken = trpc.watchlist.revokeExtensionToken.useMutation({ onSuccess: () => { setExtensionToken(null); toast.success("Private Brave token revoked"); }, onError: error => toast.error(error.message) });
  ...
  const filteredEntries = useMemo(() => entries.filter(entry => matchesWatchlistFilters(entry, { searchQuery, typeFilter, statusFilter, genreFilter, monthFilter })), [entries, searchQuery, typeFilter, statusFilter, genreFilter, monthFilter]);

  const allGenres = useMemo(() => getCategoryLabels(entries), [entries]);
  const allMonths = useMemo(() => getMonthKeys(entries), [entries]);
  ...
  const clearFilters = () => { setSearchQuery(""); setTypeFilter("all"); setStatusFilter("all"); setGenreFilter("all"); setMonthFilter("all"); };
  const allFilteredSelected = filteredEntries.length > 0 && filteredEntries.every(entry => selectedIds.has(entry.id));
  const toggleSelectAll = () => setSelectedIds(allFilteredSelected ? new Set() : new Set(filteredEntries.map(entry => entry.id)));
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-10">
          ...
          <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-[#e3e1da] bg-[#fbfaf7] p-3 lg:flex-row lg:items-center">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:w-[620px]"><Select value={typeFilter} onValueChange={value => setTypeFilter(value as MediaType | "all")}><SelectTrigger className="h-11 border-[#e4e2db] bg-white text-xs"><SelectValue placeholder="All types" /></SelectTrigger><SelectContent><SelectItem value="all">All types</SelectItem>{mediaTypes.map(type => <SelectItem key={type} value={type}>{type}</SelectItem>)}</SelectContent></Select><Select value={statusFilter} onValueChange={value => setStatusFilter(value as WatchStatus | "all")}><SelectTrigger className="h-11 border-[#e4e2db] bg-white text-xs"><SelectValue placeholder="All statuses" /></SelectTrigger><SelectContent><SelectItem value="all">All statuses</SelectItem>{watchStatuses.map(status => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select><Select value={genreFilter} onValueChange={setGenreFilter}><SelectTrigger className="h-11 border-[#e4e2db] bg-white text-xs"><SelectValue placeholder="All categories" /></SelectTrigger><SelectContent><SelectItem value="all">All categories</SelectItem>{allGenres.map(genre => <SelectItem key={genre} value={genre}>{genre}</SelectItem>)}</SelectContent></Select><Select value={monthFilter} onValueChange={setMonthFilter}><SelectTrigger className="h-11 border-[#e4e2db] bg-white text-xs"><SelectValue placeholder="All months" /></SelectTrigger><SelectContent><SelectItem value="all">All months</SelectItem>{allMonths.map(month => <SelectItem key={month} value={month}>{labelForMonth(month)}</SelectItem>)}</SelectContent></Select></div>
            {(searchQuery || typeFilter !== "all" || statusFilter !== "all" || genreFilter !== "all" || monthFilter !== "all") && <Button variant="ghost" onClick={clearFilters} className="h-10 shrink-0 text-xs text-[#69756e] hover:bg-[#eef1eb]"><FilterX className="mr-1.5 h-3.5 w-3.5" /> Clear</Button>}
          </div>
        </section>

        <section className="mt-5 flex items-center justify-between gap-4"><p className="text-xs text-[#7c8780]">{selectedIds.size ? `${selectedIds.size} titles selected locally` : "Select titles to keep a temporary visual shortlist"}</p><Button variant="outline" onClick={toggleSelectAll} className="h-9 rounded-full border-[#d4ddd2] text-xs">{allFilteredSelected ? "Clear selection" : "Select all matching"}</Button></section>

        <section className="mt-5 space-y-9">
          {groups.map(([group, groupEntries]) => <div key={group}>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{groupEntries.map(entry => <article key={entry.id} className="entry-card group relative rounded-2xl border border-[#e0e1d9] bg-[#fbfaf7] p-5 transition-all hover:-translate-y-0.5 hover:border-[#ced7cd] hover:shadow-[0_10px_24px_rgba(49,67,60,0.07)]">
              <div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><Checkbox checked={selectedIds.has(entry.id)} onCheckedChange={() => toggleSelected(entry.id)} aria-label={`Select ${entry.title}`} /><Badge variant="outline" className="rounded-full border-[#d9dfd6] bg-[#f6f7f3] px-2.5 py-1 text-[10px] font-bold tracking-wide text-[#69766f]">{entry.mediaType}</Badge></div><div className="flex opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100"><Button variant="ghost" size="icon" onClick={() => setEditing(entry)} className="h-8 w-8 text-[#5e6b64] hover:bg-[#edf0e9]"><Edit3 className="h-3.5 w-3.5" /><span className="sr-only">Edit {entry.title}</span></Button><Button variant="ghost" size="icon" onClick={() => setDeleting(entry)} className="h-8 w-8 text-[#9a6355] hover:bg-[#f7eae5]"><Trash2 className="h-3.5 w-3.5" /><span className="sr-only">Delete {entry.title}</span></Button></div></div>
